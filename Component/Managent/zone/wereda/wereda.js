...
import ZoneCreate from "./Content"
import {
  BrowserRouter,
  Switch,
  Route,
...
import WeredaCreate from "./Content"
import WeredaView from "./weredaView"
import {fetchWereda} from "../../../../Action/index"
// import ZoneView from "./zoneView"
// import zoneComponent from "./viewzone"
const styles = (theme) => ({
  root: { flexGrow: 1 },
  paper: {
...
function Content(props) {
  ...
  // console.log("zone data  ", props.weredaData)
  // props.weredaData.zone.map((i) => {
  //   console.log(i.name)
  // })
  // const zoneCard = props.weredaData.zone.map((i) => {
  //   return <zoneComponent {...i} />
  // })
  const mynewArray = [];
  const mynewArray1 = [];
    ...
    {isLoding ? <div>loding</div> : <div>
      ...
      >
        <Tabs value={value} textColor="inherit" onChange={handleChange} >
          {mynewArray1.map(i => {
            if (i.roleName === "wereda" && i.delete)
              return <Tab textColor="inherit" label="View"  {...a11yProps(1)} />
          })}
          {mynewArray1.map(i => {
            if (i.roleName === "wereda" && i.create)
          ...
          {mynewArray1.map(i => {
            if (i.roleName === "wereda" && i.update)
              return <Tab textColor="inherit" label="Update" />
          })}
          {mynewArray1.map(i => {
            if (i.roleName === "wereda" && i.select) {
