...
import {fetchZone} from "../../../../Action/index"
...
function Content(props) {
  ...
  console.log("zone data  ", props.zoneData)
  props.zoneData.zone.map((i) => {
    console.log(i.name)
  })
  // const zoneCard = props.zoneData.zone.map((i) => {
    ...
    {isLoding ? <div>loding</div> : <div>
      ...
      >
        <Tabs value={value} textColor="inherit" onChange={handleChange} >
          {mynewArray1.map(i => {
            if (i.roleName === "zone" && i.delete)
              return <Tab textColor="inherit" label="View"  {...a11yProps(1)} />
          })}
          {mynewArray1.map(i => {
            if (i.roleName === "zone" && i.create)
          ...
          {mynewArray1.map(i => {
            if (i.roleName === "zone" && i.update)
              return <Tab textColor="inherit" label="Update" />
          })}
          {mynewArray1.map(i => {
            if (i.roleName === "zone" && i.select) {
